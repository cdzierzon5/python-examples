...
# the turn does not advace properly because you add to the turn before checking
...
class Player(object):
    ...
    def __init__(self,name,num):
        ...
        self.position = -1
        self.player_num = num
        self.token = random.choice(Player.tokens)
        Player.tokens.remove(self.token)
...
class space(object):
    ...
    def move_player(self,player,board):
        ...
        if self.move > 0:
            ...
            input("press enter to slide down the the chute")
...
def main():
    ...
    # print(name)
    # num = input("what player number are you?(1-4) ")
    # print("welcome player", num)
    # player = Player(name, num)
    # print("your name:", player.name)
    # print("your token will be :", player.token)
    # print("your position on the board is:", player.position)
    # player.roll()
    # player = Player("Cody", 1)
    # board = board()
    # board.create_spaces(player,board)
    # print(board.board)
    # for i in range(100):
    #     p1p.append(Empty)
    #     p2p.append(Empty)
